        ...
        for (let i = 0; i < groups.length; i++) {
          ...
          stompClient.subscribe("/topic/messages/group/" + groups[i]["id"], function (response) {
            ...
            if (selectedUserOrGrup == data.groupId) {
              ...
              $("#userGroupAppender_" + data.groupId).append(
                '<span id="newMessage_' + data.groupId + '" style="color: red">+1</span>'
              );
  ...
  $.get(url + "/fetchAllUsers/" + userId, function (response) {
    ...
    for (let i = 0; i < users.length; i++) {
      ...
      usersTemplateHTML =
        ...
        "</div>" +
        '<div class="user_info" id="userNameAppender_' +
        users[i]["id"] +
        '">' +
        "<span>" +
        users[i]["name"] +
  ...
  $.get(url + "/fetchAllGroups/" + userId, function (response) {
    ...
    for (let i = 0; i < groups.length; i++) {
      ...
      groupsTemplateHTML =
        ...
        '<span class="online_icon"></span>' +
        "</div>" +
        '<div class="user_info" id="userGroupAppender_' +
        groups[i]["id"] +
        '">' +
        "<span>" +
        groups[i]["group_name"] +
...
function sendMsgGroup(from, text) {
  stompClient.send(
    "/app/chat/group/" + selectedUserOrGrup,
    {},
    JSON.stringify({
      fromLogin: from,
      message: text,
...
function formMessageLauch(id, name, type) {
  ...
  let submitButton =
    ...
    '<button class="input-group-text send_btn" onclick="sendMessage(\'' +
    dataType +
